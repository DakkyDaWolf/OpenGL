...
)
{
	...
	for( i = width * height; i > 0; --i )
	{
		...
		iv = (m != 0.0f) ? (unsigned char)(255.0f / m) : 1;
		iv = (iv < 1) ? 1 : iv;
		img[3] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * r + 0.5f);
		img[0] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * g + 0.5f);
		img[1] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * b + 0.5f);
		img[2] = (iv > 255) ? 255 : iv;
...
)
{
	...
	for( i = width * height; i > 0; --i )
	{
		...
		m = (b > m) ? b : m;
		/* and encode it into RGBdivA */
		iv = (m != 0.0f) ? (unsigned char)sqrtf( 255.0f * 255.0f / m ) : 1;
		iv = (iv < 1) ? 1 : iv;
		img[3] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * img[3] * r / 255.0f + 0.5f);
		img[0] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * img[3] * g / 255.0f + 0.5f);
		img[1] = (iv > 255) ? 255 : iv;
		iv = (unsigned char)(img[3] * img[3] * b / 255.0f + 0.5f);
		img[2] = (iv > 255) ? 255 : iv;
		/* and on to the next pixel */
		img += 4;
	}
